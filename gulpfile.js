...
gulp.task("sass", () => {
  ...
  return gulp
    .src(["src/sass/main.scss", "src/sass/myaccount.scss"])
    .pipe(sass().on("error", sass.logError))
    .pipe(
      postcss(plugins, { map: config.production ? false : { inline: true } })
    )
...
gulp.task("ejs", () => {
  return gulp
    .src(["views/**/*.ejs", "partials/**/*.ejs"])
    .pipe(reload({ stream: true }));
});

gulp.task("copy", () => {
  return gulp
    .src("src/font/fontawesome/**")
...
gulp.task("watch", () => {
  ...
  gulp.watch(
    ["views/**/*.ejs", "partials/**/*.ejs"],
    gulp.series(["ejs", "bs-reload"])
  );
});
